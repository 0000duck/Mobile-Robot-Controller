...
void MapManager::AddHazardPoint(int pos, Position robotPos)// 8,4,2,6의 방향을 1000,0100,0010,0001로 나타낸다.
{
	...
	if (pos & UP)
	{
		setMapData(robotPos.y, robotPos.x, HAZARD);
	}
	if (pos & LEFT)
	{
...
{
	MapNode** Map = mapModel->getMapNode();
	Map[mapHeight - 1 -y][x].isDetected = isDetected;
	Map[mapHeight - 1 - y][x].position.y = y;
	Map[mapHeight - 1 -y][x].position.x = x;
	Map[mapHeight - 1 - y][x].isSensed = isSensed;
	Map[mapHeight - 1 - y][x].data.kind = kind;
}