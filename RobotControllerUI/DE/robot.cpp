...
void Robot::Move()
{
	MoveManager* p = MoveManager::GetManager();
	switch (rDirection)
	{
	case UP:
		rPosition.y++;
		p->CSInterfacel->Move(p->CSInterfacel->CS_UP);
		break;
	case DOWN:
		rPosition.y--;
		p->CSInterfacel->Move(p->CSInterfacel->CS_DOWN);
		break;
	case LEFT:
		rPosition.x--;
		p->CSInterfacel->Move(p->CSInterfacel->CS_LEFT);
		break;
	case RIGHT:
		rPosition.x++;
		p->CSInterfacel->Move(p->CSInterfacel->CS_RIGHT);
		break;
	}
}
