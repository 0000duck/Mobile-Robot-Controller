...
void MapManager::AddColorBlob(int pos, Position robotPos)// 8,4,2,6의 방향을 1000,0100,0010,0001로 나타낸다.
{
	...
	if (pos & UP)
	{
		setMapData(robotPos.y+1, robotPos.x, COLORBLOB);
		MoveManager::GetManager()->CSInterfacel->CreateObject(3, robotPos.x,robotPos.y+1);
	}
	if (pos & LEFT)
	{
		setMapData(robotPos.y, robotPos.x - 1, COLORBLOB);
		MoveManager::GetManager()->CSInterfacel->CreateObject(3, robotPos.x-1, robotPos.y);
	}
	if (pos & DOWN)
	{
		setMapData(robotPos.y-1, robotPos.x, COLORBLOB);
...
void MapManager::AddHazardPoint(int pos, Position robotPos)// 8,4,2,6의 방향을 1000,0100,0010,0001로 나타낸다.
{
	...
	if (pos & UP)
	{
		...
		MoveManager::GetManager()->CSInterfacel->CreateObject(2, robotPos.x, robotPos.y + 1);
	}
	if (pos & LEFT)
	{
		setMapData(robotPos.y, robotPos.x - 1, HAZARD);
		MoveManager::GetManager()->CSInterfacel->CreateObject(2, robotPos.x - 1, robotPos.y);
	}
	if (pos & DOWN)
	{
		setMapData(robotPos.y - 1, robotPos.x, HAZARD);
